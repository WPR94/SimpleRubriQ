...
async function fetchDashboardStats(userId: string): Promise<{ stats: DashboardStats; recentFeedback: RecentFeedback[] }> {
  // Wrap the entire operation in a timeout (10s)
...
export function useDashboardStats(): UseQueryResult<{ stats: DashboardStats; recentFeedback: RecentFeedback[] }, Error> {
  ...
  return useQuery(
    ...
    {
      enabled: !!user,
      staleTime: 5 * 60_000, // Increase stale time to 5 minutes to reduce background refetches
      cacheTime: 10 * 60_000,
      refetchOnWindowFocus: false, // Disable refetch on window focus to prevent "blank" loading states
      retry: 1,
    }
  );
}
...
export function useFeedbackData(): UseQueryResult<FeedbackData[], Error> {
  ...
  return useQuery(
    ...
    {
      enabled: !!user,
      staleTime: 5 * 60_000, // Increase stale time to 5 minutes
      cacheTime: 10 * 60_000,
